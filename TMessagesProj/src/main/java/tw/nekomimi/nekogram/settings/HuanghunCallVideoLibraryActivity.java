 ...
 */
public class HuanghunCallVideoLibraryActivity extends BaseFragment {
    ...
    private final ArrayList<HuanghunCallVideoLibraryHelper.VideoItem> items = new ArrayList<>();
    private final Set<String> selectedPaths = new HashSet<>();

    public HuanghunCallVideoLibraryActivity(boolean deleteMode) {
    ...
    public View createView(Context context) {
        ...
        emptyView.setText(deleteMode ? "暂无可删除的通话内置视频\n请先在“选取通话内置视频”中导入视频。" : "暂无通话内置视频\n请返回通话专区选取一个或多个视频。 ");
    ...
    private void reloadItems() {
        ...
        items.clear();
        items.addAll(HuanghunCallVideoLibraryHelper.getVideoItems(getParentActivity(), currentAccount));
        Set<String> validPaths = new HashSet<>();
        for (HuanghunCallVideoLibraryHelper.VideoItem item : items) {
    ...
    private void confirmDeleteSelected() {
        ...
        showDialog(new AlertDialog.Builder(getParentActivity(), getResourceProvider())
                .setTitle("确认删除")
                .setMessage("确定删除已选的 " + count + " 个通话内置视频吗？删除后无法恢复，但不会影响手机相册中的原始文件。")
                .setNegativeButton("取消", null)
                .setPositiveButton("删除", (dialog, which) -> {
                    HuanghunCallVideoLibraryHelper.deleteVideos(getParentActivity(), currentAccount, new ArrayList<>(selectedPaths));
                    selectedPaths.clear();
                    reloadItems();
                    BulletinFactory.of(HuanghunCallVideoLibraryActivity.this).createSimpleBulletin(R.raw.done, "已删除 " + count + " 个通话内置视频。").show();
                })
                .create());
    }
    ...
    private final class VideoAdapter extends RecyclerListView.SelectionAdapter {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            HuanghunCallVideoLibraryHelper.VideoItem item = items.get(position);
    ...
    private final class VideoCell extends FrameLayout {
        ...
        private void bind(HuanghunCallVideoLibraryHelper.VideoItem item, boolean selectable, boolean checked, boolean divider) {
            boundPath = item.path;
